...
# Dataset theta_memory_load_01

Dataset description / generation: [readme.md](datasets/theta_memory_load_01/readme.md) / [generate.py](datasets/theta_memory_load_01/generate.py)

|Prompt|Model|Pass/Fail|Quoted Conclusion|Report|
|---|---|---|---|---|
|prompt-1|chatgpt-4o-latest|Pass|Momentary WM load is weakly correlated with theta power from frontal‑midline electrodes, but the effect is not strong or robust. Multivariate theta patterns do not enhance prediction accuracy and may introduce noise or overfitting.|[report.md](tests/theta_memory_load_01/prompt-1/chatgpt-4o-latest/working/report.md)|
|prompt-1|gpt-4.1|Fail|Theta power at frontal-midline electrode Fz robustly tracks WM load (significant correlation, see plot and table).Multivariate theta patterns provide a statistically significant improvement in predicting WM load over single-channel measures (t-test $p < 0.05$).|[report.md](tests/theta_memory_load_01/prompt-1/gpt-4.1/working/report.md)|
|prompt-1|claude-3.5-sonnet|Fail|These findings provide comprehensive evidence for theta oscillations as a reliable neural marker of working memory load, with both spatial and temporal characteristics supporting their utility in cognitive monitoring applications.|[report.md](tests/theta_memory_load_01/prompt-1/claude-3.5-sonnet/working/report.md)|
